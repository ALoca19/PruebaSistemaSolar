		...
		document.addEventListener('keydown', (event) => {
			switch (event.code) {
				...
				case 'Enter':
					...
					console.log(
						`Posición de la cámara: x=${pos.x.toFixed(
							2
						)}, y=${pos.y.toFixed(2)}, z=${pos.z.toFixed(2)}`
					);
					console.log(
						`Rotación de la cámara: x=${this.camera.rotation.x.toFixed(
							2
						)}, y=${this.camera.rotation.y.toFixed(
							2
	...
	startCameraSequence() {
		...
		planetPositions.forEach((planetInfo, index) => {
			...
			if (planetInfo.name === 'Jupiter' || planetInfo.name === 'Saturno') {
				...
				planetInfo.name === 'Urano' ||
				planetInfo.name === 'Neptuno'
			) {
				posicionXPlus = 6;
			} else {
				posicionXPlus = 1;
				...
				.to(
					{
						x:
							...
							(planetInfo.name === 'Sol' ? 2 : posicionXPlus),
